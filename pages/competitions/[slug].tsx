...
import { getCompetitions, getCompetitionBySlug, type Competition } from "@/data/competitions";
...
export default function CompetitionPage({ competition }: { competition: Competition }) {
  ...
  const allImages = [competition.imageUrl, ...(competition.images ?? [])].filter(Boolean) as string[];
      ...
      <Layout>
        <div style={{ borderBottom: "1px solid #1a1a1a", padding: "12px 0" }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-2" style={{ color: "#666", fontSize: "13px" }}>
            <Link href="/" style={{ color: "#888" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#c9a84c")} onMouseLeave={(e) => (e.currentTarget.style.color = "#888")}>Home</Link>
            <ChevronRight size={12} />
            <Link href="/#competitions" style={{ color: "#888" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#c9a84c")} onMouseLeave={(e) => (e.currentTarget.style.color = "#888")}>Competitions</Link>
            <ChevronRight size={12} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
              <div style={{ position: "relative", height: "420px", background: "#111", borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(201,168,76,0.15)", marginBottom: "12px" }}>
                {allImages[activeImage] ? (
                  <Image src={allImages[activeImage]} alt={competition.title} fill style={{ objectFit: "cover" }} priority />
                ) : (
                ...
                <div style={{ position: "absolute", top: "16px", left: "16px", background: "rgba(13,13,13,0.85)", border: "1px solid rgba(201,168,76,0.4)", borderRadius: "6px", padding: "5px 12px" }}>
                  <span style={{ color: "#c9a84c", fontSize: "14px", fontWeight: 700 }}>Prize Value: £{competition.prizeValue.toLocaleString()}</span>
                </div>
              </div>
              {allImages.length > 1 && (
                <div className="flex gap-2 flex-wrap">
                  {allImages.map((img, i) => (
                    <button key={i} onClick={() => setActiveImage(i)} style={{ position: "relative", width: "72px", height: "72px", borderRadius: "8px", overflow: "hidden", border: `2px solid ${i === activeImage ? "#c9a84c" : "transparent"}`, background: "#111", cursor: "pointer" }}>
            ...
            <div>
              ...
              <p style={{ color: "#aaa", fontSize: "16px", lineHeight: "1.7", marginBottom: "24px" }}>{competition.description}</p>
              ...
              <div style={{ marginBottom: "24px" }}>
                ...
                <div className="progress-bar"><div className="progress-bar-fill" style={{ width: `${soldPercent}%` }} /></div>
                <div style={{ textAlign: "right", color: "#555", fontSize: "11px", marginTop: "4px" }}>{soldPercent}% sold of {competition.maxTickets.toLocaleString()} total</div>
              </div>

              <div className="glass-card" style={{ padding: "24px", marginBottom: "20px" }}>
                <div style={{ marginBottom: "16px" }}>
                  <span style={{ color: "#c9a84c", fontSize: "28px", fontWeight: 800 }}>£{competition.ticketPrice.toFixed(2)}</span>
                ...
                <div className="flex items-center gap-4" style={{ marginBottom: "16px" }}>
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} style={{ width: "36px", height: "36px", background: "#1a1a1a", border: "1px solid #333", borderRadius: "8px", color: "#f5f0e8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Minus size={16} />
                    </button>
                    <input type="number" min={1} max={maxQty} value={quantity} onChange={(e) => setQuantity(Math.min(Math.max(1, parseInt(e.target.value) || 1), maxQty))} style={{ width: "64px", textAlign: "center", background: "#111", border: "1px solid #333", borderRadius: "8px", color: "#f5f0e8", fontSize: "16px", fontWeight: 600, padding: "8px 4px" }} />
                    <button onClick={() => setQuantity(Math.min(maxQty, quantity + 1))} style={{ width: "36px", height: "36px", background: "#1a1a1a", border: "1px solid #333", borderRadius: "8px", color: "#f5f0e8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Plus size={16} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex justify-between" style={{ marginBottom: "20px", padding: "12px 16px", background: "#111", borderRadius: "8px" }}>
                  ...
                  <span style={{ color: "#c9a84c", fontWeight: 700, fontSize: "18px" }}>£{(quantity * competition.ticketPrice).toFixed(2)}</span>
                </div>

                {remaining === 0 ? (
                  <button className="btn-gold" style={{ width: "100%", opacity: 0.4, cursor: "not-allowed" }} disabled>Sold Out</button>
                ) : (
                  <button className="btn-gold" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }} onClick={handleAdd}>
          ...
          <div style={{ marginTop: "48px", background: "rgba(201,168,76,0.05)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "12px", padding: "24px 28px" }}>
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 style={{ color: "#f5f0e8", fontWeight: 600, marginBottom: "8px" }}>Free Postal Entry Available</h4>
                <p style={{ color: "#888", fontSize: "14px", lineHeight: "1.6" }}>
                  No purchase necessary. You can enter this competition for free by post. Write your full name, email, phone, and competition name (<strong style={{ color: "#c9a84c" }}>{competition.title}</strong>) on a postcard and send to our address.{" "}
                  <Link href="/postal-entry" style={{ color: "#c9a84c" }}>Learn more →</Link>
                </p>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: "24px", padding: "20px 28px", background: "#111", borderRadius: "12px" }}>
            <h4 style={{ color: "#c9a84c", fontSize: "13px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1px", marginBottom: "10px" }}>Competition Terms</h4>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const competition = getCompetitionBySlug(params?.slug as string);
